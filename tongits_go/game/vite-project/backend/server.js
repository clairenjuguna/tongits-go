...
const calculateWinRate = (username) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT wins, losses FROM users WHERE username = ?', [username], (err, row) => {
            if (err) {
                reject(err);
                return;
            }
...
// Function to determine if player should win based on win rate
const shouldPlayerWin = async (username) => {
    const winRate = await calculateWinRate(username);
    return winRate < 80 ? Math.random() < 0.8 : Math.random() < 0.2;
};
...
io.on('connection', (socket) => {
  ...
  socket.on('findMatch', async (username) => {
    ...
    if (waitingPlayers.length > 0) {
      ...
      const playerShouldWin = await shouldPlayerWin(username);
      
      io.to(room).emit('matchFound', { 
        player1: username, 
        player2: opponent.username, 
        room,
        playerAdvantage: playerShouldWin
      });
